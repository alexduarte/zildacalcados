<html>
	...
	<form action="<?php echo Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI'])?>&id_tab=7&section=configServicosCorreios" method="post" class="form" id="configServicosCorreios">
		...
		<div class="fkcorreios_opcoes">
			...
			<input id="fkcorreios_button_ajuda" name="fkcorreios_button_ajuda" type="button" value="" onClick="window.open('<?php echo _MODULE_DIR_?>/fkcorreioslite/ajuda/servicos_correios.html','Janela','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=500,left=500,top=150'); return false;">
			<p>Ajuda</p>
	...
	<form action="<?php echo Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI'])?>&id_tab=7&section=configServicosCorreios&id_correios_transp=<?php echo $reg_correios_transp['id']?>" method="post" class="form" id="configServicosCorreios" enctype="multipart/form-data">
		...
		<div class="fkcorreios_margin_form" id="fkcorreios_correios_selecionados">
            ...
            <div class="fkcorreios_toggle_itens" id="fkcorreios_correios_selecionados_itens_<?php echo $reg_correios_transp['id'];?>">
                ...
                $sql_regioes_precos = 'SELECT `id`, `regiao_uf`, `regiao_cep` FROM `'._DB_PREFIX_.'fkcorreios_regioes_precos` WHERE `id_correios_transp` = '.(int)$reg_correios_transp['id'];
                ...
                <div class="fkcorreios_estados_atendidos">
                    ...
                    <div>
                        <input disabled type="checkbox" class="fkcorreios_correios_uf_<?php echo $reg_regioes_precos['id'];?>" name="fkcorreios_correios_uf_<?php echo $reg_regioes_precos['id'].'[]';?>" value="<?php echo $estado['estado'];?>" checked="checked"/><?php echo ' '.$estado['estado'];?>
                    </div>
                ...
                <div class="fkcorreios_form_group">
                    ...
                    <input disabled class="fkcorreios_text_cep" type="text" size="10" id="fkcorreios_correios_cep1_<?php echo $reg_regioes_precos['id'];?>" name="fkcorreios_correios_cep1_<?php echo $reg_regioes_precos['id'];?>" value=""/>
                    a
                    <input disabled class="fkcorreios_text_cep" type="text" size="10" id="fkcorreios_correios_cep2_<?php echo $reg_regioes_precos['id'];?>" name="fkcorreios_correios_cep2_<?php echo $reg_regioes_precos['id'];?>" value=""/>
                    <input disabled class="fkcorreios_button" name="button" type="button" value="<?php echo $this->l('Incluir');?>">
                </div>

                <div class="fkcorreios_form_group">
                    <label></label>
                    <textarea style="width: 492px;" disabled rows="4" id="fkcorreios_correios_intervalos_cep_<?php echo $reg_regioes_precos['id'];?>" name="fkcorreios_correios_intervalos_cep_<?php echo $reg_regioes_precos['id'];?>"><?php echo (!Tools::getValue('fkcorreios_correios_intervalos_cep_'.$reg_regioes_precos['id']) ? $reg_regioes_precos['regiao_cep'] : Tools::getValue('fkcorreios_correios_intervalos_cep_'.$reg_regioes_precos['id']));?></textarea>
                </div>
