...
class fkcorreioslite extends CarrierModule {
	...
	public function install() {
		...
		if (!parent::install()
            ...
            Or !Configuration::updateValue('FKCORREIOS_CEP_CIDADE', '')
            Or !Configuration::updateValue('FKCORREIOS_MAO_PROPRIA', '')
            Or !Configuration::updateValue('FKCORREIOS_VALOR_DECLARADO', '')
            Or !Configuration::updateValue('FKCORREIOS_AVISO_RECEBIMENTO', '')
            Or !Configuration::updateValue('FKCORREIOS_CALCULO_SERV_ADIC', '1')
			Or !Configuration::updateValue('FKCORREIOS_BLOCO_PRODUTO', 'on')
			Or !Configuration::updateValue('FKCORREIOS_BLOCO_CARRINHO', 'on')
			Or !Configuration::updateValue('FKCORREIOS_TEMPO_PREPARACAO', '0')
			Or !Configuration::updateValue('FKCORREIOS_CUSTOS_ENVIO', '0')
			Or !Configuration::updateValue('FKCORREIOS_EMBALAGEM', '0')
			Or !Configuration::updateValue('FKCORREIOS_OFFLINE', '')
            Or !Configuration::updateValue('FKCORREIOS_FRETE_GRATIS_TRANSP', '')
            Or !Configuration::updateValue('FKCORREIOS_CALCULO_LOGADO', '')) {
	...
	public function uninstall() {
		...
		if (!parent::uninstall()
			...
			Or !Configuration::deleteByName('FKCORREIOS_REFERENCIA')
            Or !Configuration::deleteByName('FKCORREIOS_DOMINIO')
            Or !Configuration::deleteByName('FKCORREIOS_PROPRIETARIO')
			Or !Configuration::deleteByName('FKCORREIOS_MEU_CEP')
            Or !Configuration::deleteByName('FKCORREIOS_CEP_CIDADE')
            Or !Configuration::deleteByName('FKCORREIOS_MAO_PROPRIA')
            Or !Configuration::deleteByName('FKCORREIOS_VALOR_DECLARADO')
            Or !Configuration::deleteByName('FKCORREIOS_AVISO_RECEBIMENTO')
            Or !Configuration::deleteByName('FKCORREIOS_CALCULO_SERV_ADIC')
			Or !Configuration::deleteByName('FKCORREIOS_BLOCO_PRODUTO')
			Or !Configuration::deleteByName('FKCORREIOS_BLOCO_CARRINHO')
			Or !Configuration::deleteByName('FKCORREIOS_TEMPO_PREPARACAO')
			Or !Configuration::deleteByName('FKCORREIOS_CUSTOS_ENVIO')
			Or !Configuration::deleteByName('FKCORREIOS_EMBALAGEM')
			Or !Configuration::deleteByName('FKCORREIOS_OFFLINE')
            Or !Configuration::deleteByName('FKCORREIOS_FRETE_GRATIS_TRANSP')
            Or !Configuration::deleteByName('FKCORREIOS_CALCULO_LOGADO')) {
	...
	public function getContent() {
		...
		$this->_html = '';
		$this->_html .= '<h2>'.$this->l('Módulo FKcorreios').'</h2>';
		...
		if ($servicos_correios) {
			...
			foreach ($servicos_correios as $servico) {
				...
				if ($especif_correios) {
					...
					// Insere registro em fkcorreios_regioes_precos
					$dados = array(
					...
					Db::getInstance()->insert('fkcorreios_regioes_precos', $dados);
				}
			}
		}
	}
	...
	private function excluiServicosCorreios() {
		...
		if ($carrier->delete()) {
			...
			Db::getInstance()->delete('fkcorreios_regioes_precos', '`id_correios_transp` = '.(int)$id_correios_transp);
            Db::getInstance()->delete('fkcorreios_tabelas_offline', '`id_correios_transp` = '.(int)$id_correios_transp);
	...
	private function salvaServicosCorreios() {
		...
		// Altera fkcorreios_regioes_precos
		$regioes_precos = Db::getInstance()->executeS('SELECT `id` FROM `'._DB_PREFIX_.'fkcorreios_regioes_precos` WHERE `id_correios_transp` = '.(int)$id_correios_transp);
		...
		foreach ($regioes_precos as $reg_regioes_precos) {
			...
			Db::getInstance()->update('fkcorreios_regioes_precos', $dados, 'id = '.(int)$reg_regioes_precos['id']);
	...
	private function displayForm() {
		...
		$this->_html .= '<fieldset>';
		$this->_html .= '<legend><img src="'.$this->_path.'logo.gif" alt="" /> '.$this->l('Status do Módulo FKcorreios').'</legend>';
		...
		if (Configuration::get('FKCORREIOS_REFERENCIA') == '' Or Configuration::get('FKCORREIOS_DOMINIO') == '' Or Configuration::get('FKCORREIOS_PROPRIETARIO') == '') {
			...
			if ($enviarAlert == false) {
				$this->_html .= '<img src="'._PS_IMG_ .'admin/module_install.png" /><strong>'.$this->l('FKcorreios está configurado e online!').'</strong>';
			}else {
				$this->_html .= '<strong>'.$this->l('FKcorreios ainda não configurado, por favor verifique os alertas abaixo:').'</strong>';
				$this->_html .= '<br><br>';
	...
	private function displayFormConfig() {
		...
		if (Configuration::get('FKCORREIOS_REFERENCIA') != '' and Configuration::get('FKCORREIOS_DOMINIO') != '' and Configuration::get('FKCORREIOS_PROPRIETARIO') != '') {
			...
			if (!$this->wsVerificaLicenca(Configuration::get('FKCORREIOS_REFERENCIA'), Tools::getShopDomain(false,true))) {
				...
				$this->_html .= '   <li id="menuTab4" class="menuTabButton">4. '.$id_prazos_entrega.'</li>';
				$this->_html .= '   <li id="menuTab5" class="menuTabButton">5. '.$id_embalagens.'</li>';
				$this->_html .= '   <li id="menuTab6" class="menuTabButton">6. '.$id_especificacoes_correios.'</li>';
				$this->_html .= '   <li id="menuTab7" class="menuTabButton">7. '.$id_servicos_correios.'</li>';
                $this->_html .= '   <li id="menuTab8" class="menuTabButton">8. '.$id_frete_gratis.'</li>';
                $this->_html .= '   <li id="menuTab9" class="menuTabButton">9. '.$id_tabelas_off.'</li>';
				$this->_html .= '</ul>';
                ...
                $this->_html .= '   <div id="menuTab8Sheet" class="fkcorreios_tabItem">';
                ob_start();
                ...
                $this->_html .= '   <div id="menuTab9Sheet" class="fkcorreios_tabItem">';
                ob_start();
    ...
    public function hookdisplayShoppingCartFooter($params) {
        ...
        // Inicializa array smarty
        $this->smarty->assign(array('fkcorreios_cep' => ''));
        if ($this->context->cookie->fkcorreios_cep) {
            $this->smarty->assign(array('fkcorreios_cep' => $this->context->cookie->fkcorreios_cep));
        }

        $fkcorreios_cep_msg = array('msg' => $this->l('Aguardando CEP'));
        $this->context->smarty->assign(array('fkcorreios_cep_msg' => $fkcorreios_cep_msg));

        $fkcorreios_cep_frete = array('frete' => array());

        // Calcula o frete
        if (Tools::isSubmit('submitCarrinho') Or $this->context->customer->isLogged() Or isset($this->context->cookie->fkcorreios_cep)){
            ...
            if (strlen($cep_destino) <> 8) {
                ...
                $fkcorreios_cep_msg['msg'] = $this->l('CEP inválido');
                $this->context->smarty->assign(array('fkcorreios_cep_msg' => $fkcorreios_cep_msg));

                return $this->display(__FILE__, 'carrinho.tpl');
            }
            ...
            $sql = 'SELECT `id_carrier`, `nome_carrier`
            ...
            foreach ($correios_transp as $reg) {
                ...
                $fkcorreios_cep_msg['msg'] = $this->l('Frete calculado');
                $this->context->smarty->assign(array('fkcorreios_cep_msg' => $fkcorreios_cep_msg));
                ...
                $fkcorreios_cep_frete['frete'][] = array(
            ...
            usort($fkcorreios_cep_frete['frete'], array($this, 'ordenaValorFrete'));

            $this->context->smarty->assign(array('fkcorreios_cep_frete' => $fkcorreios_cep_frete));
        ...
        if (version_compare(substr(_PS_VERSION_, 0 ,5), '1.6.0', '<')) {
            return $this->display(__FILE__, 'views/carrinho_15x.tpl');
        }else {
            return $this->display(__FILE__, 'views/carrinho_16x.tpl');
        }
    ...
    public function hookdisplayFooterProduct($params) {
        ...
        if (Tools::isSubmit('submitProd')){
            ...
            if (strlen($cep_destino) <> 8) {
                ...
                return $this->display(__FILE__, 'produto.tpl');
            }
        ...
        if (version_compare(substr(_PS_VERSION_, 0 ,5), '1.6.0', '<')) {
            return $this->display(__FILE__, 'views/produto_15x.tpl');
        }else {
            return $this->display(__FILE__, 'views/produto_16x.tpl');
        }
    ...
    private function verificaRegiaoAtendida($id_carrier, $cep_destino, $uf_destino) {
        ...
        $sql = 'SELECT  '._DB_PREFIX_.'fkcorreios_correios_transp.id_correios,
                        '._DB_PREFIX_.'fkcorreios_regioes_precos.regiao_uf
                FROM '._DB_PREFIX_.'fkcorreios_correios_transp
                    INNER JOIN '._DB_PREFIX_.'fkcorreios_regioes_precos
                    ON '._DB_PREFIX_.'fkcorreios_correios_transp.id = '._DB_PREFIX_.'fkcorreios_regioes_precos.id_correios_transp
                WHERE   '._DB_PREFIX_.'fkcorreios_correios_transp.ativo = 1 AND
                        '._DB_PREFIX_.'fkcorreios_correios_transp.id_shop = '.$this->context->shop->id.' AND
    ...
    private function verificaPedidoFreteGratis($cep_destino, $uf_destino, $valor_pedido) {
        ...
        $sql = 'SELECT  '._DB_PREFIX_.'fkcorreios_correios_transp.id_carrier,
                        '._DB_PREFIX_.'fkcorreios_frete_gratis.regiao_uf
    ...
    private function criaTabelas() {
		...
		$sql = "INSERT INTO `"._DB_PREFIX_."fkcorreios_cadastro_cep` (`estado`, `capital`, `cep_estado`, `cep_capital`, `cep_base_capital`, `cep_base_interior`) VALUES
            ...
            ('GO', 'Goiãnia', 			'72800000:72999999/73700000:76799999', 		'74000001:74899999',                    '74000_001', '75000-001'),
		...
		$sql = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'fkcorreios_prazos_entrega` (
	            `id`		 		int(10) 	NOT NULL AUTO_INCREMENT,
				`id_shop`			int(10),
	            `estado` 			varchar(2),
		...
		$sql = 'CREATE TABLE IF NOT EXISTS `' ._DB_PREFIX_. 'fkcorreios_regioes_precos` (
            	`id` 								int(10) 		NOT NULL AUTO_INCREMENT,
        ...
        $sql = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'fkcorreios_tabelas_offline` (
                `id`                    int(10)     NOT NULL AUTO_INCREMENT,
                `id_correios_transp`    int(10),
	...
	private function excluiTabelas() {
		...
		$sql = "DROP TABLE IF EXISTS `"._DB_PREFIX_."fkcorreios_prazos_entrega`;";
		$db-> Execute($sql);
		...
		$sql = "DROP TABLE IF EXISTS `"._DB_PREFIX_."fkcorreios_regioes_precos`;";
		$db-> Execute($sql);

        $sql = "DROP TABLE IF EXISTS `"._DB_PREFIX_."fkcorreios_frete_gratis`;";
        $db-> Execute($sql);

        $sql = "DROP TABLE IF EXISTS `"._DB_PREFIX_."fkcorreios_tabelas_offline`;";
        $db-> Execute($sql);
